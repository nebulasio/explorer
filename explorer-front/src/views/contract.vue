...
<template>
  ...
  <div class="vue-contract " v-bind:triggedComputed="urlChange">
    <vue-bread
      v-if="obj"
      v-bind:title="obj.tokenName"
      subtitle="[ NRC20 Token ]"
    ></vue-bread>
    <div class="container" v-if="obj">
      <div class="explorer-table-container">
        ...
        <table class="explorer-table font-16 d-none d-md-table">
          <tr>
            <td class="font-color-555555 td-left pl-16">
              {{ $t("contractTotalSupplyTitle") }}
            </td>
            <td class="font-color-000000">
              {{ tokenAmount(obj.total, decimal) }} {{ obj.tokenName }}
            </td>
          </tr>
          <tr v-if="tokenPrice">
            <td class="font-color-555555 pl-16">
              {{ $t("contractPriceTitle") }}
            </td>
            <td>
              <span class="font-color-000000">${{ tokenPrice.price }}</span>
              <span
                :class="{
                  'font-color-07A656': tokenPrice.trends === 1,
                  'font-color-F04434': tokenPrice.trends != 1
                }"
                >(</span
              >
              ...
              <img
                ...
                v-if="tokenPrice.trends === 1"
                src="../../static/img/ic_exchange_rate_up.png"
              />
              <img
                class="icon16"
                style="margin-top: -4px"
                v-else
                src="../../static/img/ic_exchange_rate_down.png"
              />
              ...
              <span
                ...
                >{{ tokenPrice.change24h + "%" }})</span
              >
            </td>
          </tr>
          <tr>
            <td class="font-color-555555 pl-16">
              {{ $t("contractHoldersTitle") }}
            </td>
            <td class="font-color-000000">
              {{ numberAddComma(obj.holderCount) }}
              <span id="contractAddressesText" class="localizable"></span>
            </td>
          </tr>
          <tr>
            <td class="font-color-555555 pl-16">
              {{ $t("contractTransfersTitle") }}
            </td>
            <td class="font-color-000000">
              {{ numberAddComma(obj.transactionCount) }}
            </td>
          </tr>
          <tr>
            <td class="font-color-555555 pl-16">
              {{ $t("contractContractTitle") }}
            </td>
            <td>
        ...
        <div class="mobile-detail d-md-none">
          <div>
            <span>{{ $t("contractTotalSupplyTitle2") }}</span>
            <div class="detail">
              {{ tokenAmount(obj.total, decimal) }} {{ obj.tokenName }}
            </div>
          </div>
          <div v-if="tokenPrice">
            <span>{{ $t("contractPriceTitle2") }}</span>
            <div class="detail">
              <span class="font-color-000000">${{ tokenPrice.price }}</span>
              <span
                :class="{
          ...
          <div>
            <span>{{ $t("contractHoldersTitle2") }}</span>
            <div class="detail">
              {{ numberAddComma(obj.holderCount) }} addresses
            </div>
          </div>
          <div>
            <span>{{ $t("contractTransfersTitle2") }}</span>
            <div class="detail">{{ numberAddComma(obj.transactionCount) }}</div>
          </div>
          <div>
            <span>{{ $t("contractContractTitle2") }}</span>
            <div class="detail">
              <router-link v-bind:to="fragApi + '/address/' + obj.contract">
                <span class="monospace">{{ obj.contract }}</span>
              </router-link>
      ...
      <div class="tab" v-show="tab === 1">
        ...
        >
          <div class="col mr-auto px-0 font-16 font-bold font-color-000000">
            ...
            <span>{{ $t("contractLatestText") }}</span>
            {{ txs.length }} {{ txs.length > 1 ? "txs" : "tx" }}
            <span>{{ $t("contractFromTotalText") }}</span>
            {{ numberAddComma(obj.transactionCount) }}
            <span
              v-if="obj.transactionCount > 1"
              class="localizable"
              id="contractTransactionsText"
            ></span
            ><span
              v-else
              class="localizable"
              id="contractTransactionText"
            ></span
            ><span v-if="obj.pendingTransactionCount > 0"
              >{{ "( + " + obj.pendingTransactionCount }}
              <span v-if="obj.pendingTransactionCount == 1">
                {{ $t("contractPendingSingularText") }} </span
              ><span v-else>
                {{ $t("contractPendingPluralText") }}
              </span> </span
            >{{ obj.pendingTransactionCount > 1 ? "txs )" : "tx )" }}
          </div>
          <div class="col-auto px-0">
            <router-link
              class="d-block d-md-inline-block align-middle mt-1 mt-md-0"
              v-bind:to="fragApi + '/contract-txs?contract=' + $route.params.id"
            >
              <span class="font-16">
                {{ $t("contractViewAllText") }}
              </span>
            </router-link>
            <span v-if="obj.pendingTransactionCount > 0">
              ...
              <router-link
                ...
                v-bind:to="
                  fragApi +
                    '/contract-txs?contract=' +
                    $route.params.id +
                    '&isPending=true'
                "
              >
                <span class="font-16"
                  ><span>{{ $t("contractViewAllText2") }}</span>
                  {{ obj.pendingTransactionCount }}
                  <span>{{ $t("contractPendingText2") }}</span>
                  <span v-if="obj.pendingTransactionCount > 1">
                    {{ $t("contractPendingPluralText2") }} </span
                  ><span v-else>
                    {{ $t("contractPendingSingularText2") }}
                  </span></span
                >
              </router-link>
            </span>
        ...
        <div class="explorer-table-container">
          <table v-if="txs.length" class="mt20 explorer-table list-table">
            <tr
              class="font-color-000000 font-12 font-bold"
              style="height: 46px; background-color: #e8e8e8;"
            >
              <th></th>
              <th>{{ $t("contractTableTxHash") }}</th>
              <th>{{ $t("contractTableBlock") }}</th>
              <th>{{ $t("contractTableAge") }}</th>
              <th>{{ $t("contractTableFrom") }}</th>
              <th></th>
              <th>{{ $t("contractTableTo") }}</th>
              <th class="text-right">
                {{ $t("contractTableValue") }}
              </th>
              <th class="text-right pr-3">
                {{ $t("contractTableTxFee") }}
              </th>
            </tr>
            ...
            <tr v-for="o in txs" :key="o.hash">
              ...
              <td class="txs-block">
                ...
                <i class="font-14 font-color-000000" v-else>
                  {{ $t("contractPendingText3") }}
                </i>
              </td>
              <td class="time font-14 font-color-555555">
                <div>
                  <div>
                    <span>{{ $t("contractTimeDiffPrefix") }}</span>
                    {{ timeConversion(o.timeDiff) }}
                    <span>{{ $t("contractTimeDiffSuffix") }}</span>
                  </div>
              ...
              <td class="tdxxxwddd txs-from-to">
                ...
                <span
                  class="fromTo font-color-000000 font-14 monospace"
                  v-if="o.to === $route.params.id"
                  >{{ o.to }}</span
                >
        ...
        <div
          v-if="txs.length"
          class="d-block d-md-flex flex-row align-items-center justify-content-end mt20"
        >
          <div class="col-auto px-0">
            <router-link
              class="d-block d-md-inline-block align-middle mt-1 mt-md-0"
            ...
            >
              <span class="font-16"
                ><span>{{ $t("contractViewAllText3") }}</span>
                {{ obj.transactionCount }}
                {{ obj.transactionCount > 1 ? "txs" : "tx" }}</span
              >
            </router-link>
            <span v-if="obj.pendingTransactionCount > 0">
              <span class="d-none d-md-inline-block px-2">|</span>
              ...
              >
                <span class="font-16"
                  ><span>{{ $t("contractViewAllText4") }}</span>
                  {{ obj.pendingTransactionCount }}
                  <span v-if="obj.pendingTransactionCount > 1">
                    {{ $t("contractPendingTransactionsPlural") }} </span
                  ><span v-else>
                    {{ $t("contractPendingTransactionsSingular") }}
                  </span></span
                >
              </router-link>
            </span>
      ...
      <div class="tab" v-show="tab === 2">
        <div class="align-items-center info-and-pagination mt20 row">
          <div class="col font-16 font-bold font-color-000000">
            <span> {{ $t("contractHoldersFoundPrefix") }} </span
            >{{ numberAddComma(totalHolderCount)
            }}<span>{{ $t("contractHoldersFoundSuffix") }}</span>
          </div>
        </div>
        ...
        <div class="explorer-table-container">
          <table v-if="holders.length" class="mt20 explorer-table list-table">
            <tr class="list-header font-12 font-bold font-color-000000">
              <th style="padding-left: 24px; width: 100px;">
                {{ $t("contractTableRank") }}
              </th>
              <th style="width: 30%">
                {{ $t("contractTableAddress") }}
              </th>
              <th class="text-right">
                {{ $t("contractTableQuantity") }}
              </th>
              <th class="text-right" style="padding-right: 24px;">
                {{ $t("contractTablePercentage") }}
              </th>
            </tr>

            <tr v-for="o in holders" :key="o.address">
              <td
                class="font-color-000000 font-14 font-bold"
              ...
              <td class="tdxxxwddd">
                <router-link
                  ...
                  v-bind:to="fragApi + '/address/' + o.address"
                >
                  <span class="font-14  monospace">{{ o.address }}</span>
                </router-link>
              </td>
              <td class="text-right font-14 font-color-555555">
                {{ tokenAmount(o.balance, decimal).shortAmount() }}
              </td>
              <td
                class="text-right font-14 font-color-555555"
    ...
    urlChange() {
      ...
      api.getContract(
        ...
        o => {
          ...
          this.obj = o.contract;
          this.txs = o.txList;
          this.tokenPrice = o.price
            ? { price: o.price, trends: o.trends, change24h: o.change24h }
            : null;
        },
        xhr => {
          this.$root.showModalLoading = false;
